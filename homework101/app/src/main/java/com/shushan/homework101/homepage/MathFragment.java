...
public class MathFragment extends BaseFragment {
    ...
    private ArrayList<TeacherBean> teacherBeansMath=new ArrayList<>();
    ...
    private HomepageView homepageView = new HomepageView() {
        ...
        public void onSuccess(Homepage homepage) {
            if(teacherBeansMath!=null&&teacherBeansMath.size()>0){
                teacherBeansMath.clear();
            }
            if (homepage != null) {
                ...
                if (homepage.getData().getTeacher() != null && homepage.getData().getTeacher().size() > 0) {
                    ...
                    for (int i = 0; i < homepage.getData().getTeacher().size(); i++) {
                        if (homepage.getData().getTeacher().get(i).getSubject().equals(getResources().getString(R.string.my_math))) {
                            ...
                            teacherBeansMath.add(teacher);
                        }

                    }
                        homepageListViewAdapter = new HomepageListViewAdapter(mContext,teacherBeansMath);

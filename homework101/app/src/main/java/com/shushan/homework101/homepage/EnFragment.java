...
public class EnFragment extends BaseFragment {
    ...
    private ArrayList<TeacherBean> teacherBeansEn=new ArrayList<>();
    ...
    private HomepageView homepageView = new HomepageView() {
        ...
        public void onSuccess(Homepage homepage) {
            ...
            if(teacherBeansEn!=null&&teacherBeansEn.size()>0){
                teacherBeansEn.clear();
            }
            if (homepage != null) {
                ...
                if (homepage.getData().getTeacher() != null && homepage.getData().getTeacher().size() > 0) {
                    ...
                    for (int i = 0; i < homepage.getData().getTeacher().size(); i++) {
                        if (homepage.getData().getTeacher().get(i).getSubject().equals(getResources().getString(R.string.my_english))) {
                            ...
                            teacherBeansEn.add(teacher);

                        }

                    }
                        homepageListViewAdapter = new HomepageListViewAdapter(mContext,teacherBeansEn);

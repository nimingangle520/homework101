...
public class GDJYFragment extends BaseFragment {
    // TODO: Rename parameter arguments, choose names that match
    ListView lv_homepage;
    private HomepageListViewAdapter homepageListViewAdapter;
    private String userid;
    private String token;
    private ArrayList<TeacherBean> teacherBeansGDJY=new ArrayList<>();
    ...
    private HomepageView homepageView = new HomepageView() {
        ...
        public void onSuccess(Homepage homepage) {
            if(teacherBeansGDJY!=null&&teacherBeansGDJY.size()>0){
                teacherBeansGDJY.clear();
            }
            if (homepage != null) {
                ...
                if (homepage.getData().getTeacher() != null && homepage.getData().getTeacher().size() > 0) {
                    ...
                    for (int i = 0; i < homepage.getData().getTeacher().size(); i++) {
                        if (homepage.getData().getTeacher().get(i).getSubject().equals(getResources().getString(R.string.my_politics))) {
                            ...
                            teacherBeansGDJY.add(teacher);

                        }

                    }
                        homepageListViewAdapter = new HomepageListViewAdapter(mContext,teacherBeansGDJY);

...
public class HistoryFragment extends BaseFragment {
    ...
    private ArrayList<TeacherBean> teacherBeansHistory=new ArrayList<>();
    ...
    private HomepageView homepageView = new HomepageView() {
        ...
        public void onSuccess(Homepage homepage) {
            if(teacherBeansHistory!=null&&teacherBeansHistory.size()>0){
                teacherBeansHistory.clear();
            }
            if (homepage != null) {
                ...
                if (homepage.getData().getTeacher() != null && homepage.getData().getTeacher().size() > 0) {
                    ...
                    for (int i = 0; i < homepage.getData().getTeacher().size(); i++) {
                        if (homepage.getData().getTeacher().get(i).getSubject().equals(getResources().getString(R.string.my_history))) {
                            ...
                            teacherBeansHistory.add(teacher);

                        }

                    }
                        homepageListViewAdapter = new HomepageListViewAdapter(mContext,teacherBeansHistory);

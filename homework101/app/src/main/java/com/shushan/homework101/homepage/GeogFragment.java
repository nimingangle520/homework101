...
public class GeogFragment extends BaseFragment {
    // TODO: Rename parameter arguments, choose names that match
    ListView lv_homepage;
    private HomepageListViewAdapter homepageListViewAdapter;
    private String userid;
    private String token;
    private ArrayList<TeacherBean> teacherBeansGeog=new ArrayList<>();
    ...
    private HomepageView homepageView = new HomepageView() {
        ...
        public void onSuccess(Homepage homepage) {
            if(teacherBeansGeog!=null&&teacherBeansGeog.size()>0){
                teacherBeansGeog.clear();
            }
            if (homepage != null) {
                ...
                if (homepage.getData().getTeacher() != null && homepage.getData().getTeacher().size() > 0) {
                    ...
                    for (int i = 0; i < homepage.getData().getTeacher().size(); i++) {
                        if (homepage.getData().getTeacher().get(i).getSubject().equals(getResources().getString(R.string.my_geo))) {
                            ...
                            teacherBeansGeog.add(teacher);

                        }

                    }
                        homepageListViewAdapter = new HomepageListViewAdapter(mContext,teacherBeansGeog);
